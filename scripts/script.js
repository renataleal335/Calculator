...
class Token {
  ...
  _treatedArray(input) {
      const stepOne = this._transformArray(input);
...
class Calculator extends Token {
  constructor(input, qtdOperators) {
      super(input);
      this._qtdOperators = qtdOperators;
  }
  _countOperators() {
  ...
  _calculateValue() {
      ...
      let operators = this._qtdOperators;
      let result = '';
      let beforeOperator = '';
      let afterOperator = '';
...
function calculate(input) {
  ...
  const qtdOperators = new Calculator(tokens)._countOperators();
  const result = new Calculator(tokens, qtdOperators)._calculateValue();
  return result;
}